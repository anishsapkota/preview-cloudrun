...
export class ServiceManifest {
  ...
  public getImage(): string {
    return this._object?.spec?.template?.spec?.containers
      ? this._object.spec.template.spec.containers[0].image
      : "";
  }

  public getRevisionName(): string {
    return this._object?.spec?.template?.metadata
      ? this._object?.spec?.template?.metadata.name
      : "";
  }
  ...
  public updatePreviewTraffic(revision: string, tag: string): void {
    ...
    const traffic: Array<TrafficTarget> = this._object.spec
...
export function parseServiceManifest(data: string): ServiceManifest {
  try {
    ...
    throw new Error(
      `failed to parse service manifest: ${msg}, stdout: ${data}`
    );
  }
}
