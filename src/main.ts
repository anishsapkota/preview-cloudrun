...
import { getInput, addPath, setFailed, info, warning } from "@actions/core";
...
import { Gcloud, GcloudOptions } from "./gcloud";
...
export async function run(): Promise<void> {
  try {
    ...
    const gcloudVersion = await computeGcloudVersion(
      getInput("gcloud_version")
    );
    ...
    if (!url)
      throw new Error(`failed to get the preview url from ${service} manifest`);
...
function generateRevisionName(svc: string, image: string): string {
  ...
  if (image.includes(":"))
    version = image
      ...
      .join("");
  return (
    svc + "-" + version + "-" + event.pull_request.head.sha.substring(0, 7)
  );
}
...
function generateCommentBody(args: {
  project: string;
  service: string;
  region: string;
  commitHash: string;
  ...
  const title = `✅ Successfully created Preview Deployment.`;
